...
if (not (issquare (A.inf)))
    error ("interval:InvalidOperand", "operator \: Matrix is not square");
endif
if (rows (A.inf) ~= rows (b.inf))
    error ("interval:InvalidOperand", ["operator \: ", ...
...
if (cond == 0)
    ...
    error ("System is probably singular")
endif
