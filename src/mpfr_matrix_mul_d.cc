    ...
    for (int j = 0; j < m; j++)
      {
        ...
        for (int k = 0; k < l; k++)
          {
            const double xl = matrix_xl.elem (i, k);
            const double xu = matrix_xu.elem (i, k);
            const double yl = matrix_yl.elem (k, j);
            const double yu = matrix_yu.elem (k, j);
        ...
        result_l.elem (i, j) = mpfr_get_d (accu_l, MPFR_RNDD);
        result_u.elem (i, j) = mpfr_get_d (accu_u, MPFR_RNDU);
      }
...
DEFUN_DLD (mpfr_matrix_mul_d, args, nargout, 
  ...
  "\n"
  "Compute the upper and lower boundary of the matrix multiplication of "
  "interval matrices [@var{XL}, @var{XU}] and [@var{YL}, @var{YU}] in "
  "binary64 numbers with tightest result."
  "\n\n"
